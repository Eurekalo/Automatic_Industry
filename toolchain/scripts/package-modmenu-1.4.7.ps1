...
Compress-Archive -Path "$tempRel\*" -DestinationPath $releaseZip
Write-Host "Created release archive: $releaseZip"

Write-Host "=== 4. Staging and Creating Source ZIP Archive ==="
$sourceTargetDir = Join-Path $tempSrc "ModMenu-src-$Version"
New-Item -ItemType Directory -Path $sourceTargetDir -Force | Out-Null

Get-ChildItem -Path $srcFolder -Recurse | Where-Object {
    $_.FullName -notmatch '\\bin(\\|$)' -and
    $_.FullName -notmatch '\\obj(\\|$)' -and
    $_.FullName -notmatch '\.vs(\\|$)' -and
    $_.FullName -notmatch '\.git(\\|$)'
} | ForEach-Object {
    $relPath = $_.FullName.Substring($srcFolder.Length).TrimStart('\', '/')
    $targetPath = Join-Path $sourceTargetDir $relPath
    if ($_.PSIsContainer) {
...
Compress-Archive -Path "$tempSrc\*" -DestinationPath $srcZip
Write-Host "Created source archive: $srcZip"

Write-Host "=== 5. Deploying Mod Folder to Workspace Root ==="
$workspaceModDir = Join-Path $WorkspaceRoot "ModMenu-$Version"
...
Remove-Item -Recurse -Force $tempDir
Write-Host "Packaging completed successfully for ModMenu $Version."
